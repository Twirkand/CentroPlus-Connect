...
class LoginControllerTest {
    ...
    void handleLogin_credencialesCorrectas_estableceSesionYNavega() {
        try (MockedStatic<Session> sessionMock = mockStatic(Session.class);
            ...
            when(service.login("12345678A", "correcta")).thenReturn(user);
    ...
    void handleLogin_excepcion_muestraError() {
        ...
        when(service.login(anyString(), anyString())).thenThrow(new RuntimeException("BD caída"));
    ...
    void handleRegister_contrasenaCorta_muestraError() {
        ...
        verify(registerErrorLabel).setText("Contraseña demasiado corta");
    ...
    void handleRegister_datosValidos_creaYNavega() {
        try (MockedStatic<ScreenManager> screenMock = mockStatic(ScreenManager.class)) {
            when(registerPasswordField.getText()).thenReturn("segura123");
    ...
    void handleRegister_excepcion_muestraError() {
        ...
        when(telefonoField.getText()).thenReturn("600000000");
        when(tipoUsuarioBox.getValue()).thenReturn("SOCIO");
        doThrow(new RuntimeException("DNI duplicado")).when(service).create(any());
