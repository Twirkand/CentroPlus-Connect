...
class DetalleIncidenciaControllerTest {
    ...
    void cargarIncidencia_encontrada_rellenaCampos() {
        try (MockedStatic<ScreenManager> screenMock = mockStatic(ScreenManager.class)) {
            ...
            verify(lblAsunto).setText("Asunto de prueba");
            verify(lblEstado).setText("Estado: ABIERTA");
            verify(txtDescripcion).setText("Descripción de prueba");
        }
    }
    ...
    private void invoke(String methodName) {
        try {
            Method m = DetalleIncidenciaController.class.getDeclaredMethod(methodName);
